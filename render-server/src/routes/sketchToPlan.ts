...
router.post('/', async (req, res) => {
  ...
  const cadastralContext = (cadastralPart && compositePart)
    ? [
        ...
        '  → 지적도는 건물 배치의 최대 허용 영역과 방향 기준을 제공합니다. 대지 경계 형태를 건물 외벽으로 사용하지 마세요.',
        '- IMAGE_2: 합성 이미지 (지적도+스케치 오버레이) — 건물의 대지 내 위치·이격거리 파악용',
        '  → IMAGE_2의 스케치 스트로크가 IMAGE_1 대지 내 어느 위치·비율을 점유하는지 읽으세요.',
        '  (예: 스케치가 대지 좌상단 30% 영역을 점유하면 → 생성된 평면도도 대지 좌상단 30% 영역에 위치)',
        '- IMAGE_3: 순수 스케치 스트로크 — 건물 외곽선 및 방의 위상학적 관계(Room Topology) 분석 전용',
        '  → IMAGE_3 스트로크의 최외곽 = 건물 외벽(External Wall). 닫힌 폴리곤이 있으면 그 폴리곤이 외벽, 없으면 버블 합집합 외곽이 외벽.',
        '절대 규칙 1: IMAGE_1 대지 경계와 방향은 변경 불가. 단, 대지 경계 형태가 건물 외벽 형태를 결정하지 않습니다.',
        '절대 규칙 2: 건물 외벽은 IMAGE_3 스케치 스트로크 외곽으로 결정합니다. IMAGE_1 대지 경계를 건물 외벽으로 사용 금지.',
        '절대 규칙 3: 생성된 평면도는 IMAGE_1 대지 경계 안에 위치해야 하며, IMAGE_2에서 파악한 위치·이격거리를 반영해야 합니다.',
        '---',
      ].join('\n')
    : cadastralPart
      ? [
          ...
          '  → 지적도는 건물 배치의 최대 허용 영역과 방향 기준을 제공합니다. 대지 경계 형태를 건물 외벽으로 사용하지 마세요.',
          '- 두 번째 이미지 (IMAGE_2): 스케치 스트로크 — 건물 외곽선 및 위상학적 평면 (Room Topology)',
          '  → IMAGE_2 스트로크의 최외곽 = 건물 외벽(External Wall). 닫힌 폴리곤이 있으면 그 폴리곤이 외벽, 없으면 버블 합집합 외곽이 외벽.',
          '절대 규칙 1: IMAGE_1 대지 경계와 방향은 변경 불가. 단, 대지 경계 형태가 건물 외벽을 결정하지 않습니다.',
          '절대 규칙 2: 건물 외벽은 IMAGE_2 스케치 스트로크 외곽으로 결정합니다. IMAGE_1 대지 경계를 건물 외벽으로 사용 금지.',
          '절대 규칙 3: 생성된 평면도는 반드시 IMAGE_1 대지 경계 안에 위치해야 합니다.',
          '---',
        ].join('\n')
      : '';
