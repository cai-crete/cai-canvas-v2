...
router.post('/', async (req, res) => {
  ...
  try {
    const phase3Prompt = [
      ...
      '',
      '이중 권위 원칙 (예외 없는 절대 규칙):',
      '- 수평 차원(X-Z 평면, 배치/깊이/구역): 평면도 완전 우선 — floorplan-analysis-spec.horizontal_constants 픽셀 수준 보존',
      '- 수직 차원(Y축, 비율/파사드/층고): 입면도 우선 (Formal Authority)',
      '- 입면도 데이터가 X-Z 평면에 영향을 주는 즉시 [MFM-06] 발동 → 재생성',
      '',
      '[SPATIAL LOCK 확인]: 생성 전 floorplan-analysis-spec.horizontal_constants 전체 항목을 X-Z 골격에 고정한다.',
      '',
