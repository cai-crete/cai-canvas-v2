...
export default function PrintExpandedView({
  ...
  const handleGoBack = useCallback(() => {
    if (isDirty && saveActionRef.current) {
      setIsSaving(true);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') handleGoBack(); };
  ...
  }, [onGeneratePrintComplete, onPrintNodeUpdate, onCollapse]);
  ...
  const renderToolbarWrapper = useCallback((tools: PrintToolbarTools) => {
    saveActionRef.current = tools.onSave;
    ...
    <div style={{ flex: 1, overflow: 'hidden' }}>
      ...
      {isSaving && (
        <div style={{
          ...
          <span className="text-body-2" style={{ color: 'var(--color-gray-800)', fontWeight: 500 }}>
            문서를 저장하는 중...
          </span>
