...
- 체크리스트 항목이 모두 완료된 `docs/exec-plans/active/` 파일은 `docs/exec-plans/completed/`로 이동 후 `active/`에서 삭제하세요
  - 당일 기준 24시간 이상 지난 파일은 `docs/exec-plans/completed/old/` 폴더로 이동하세요 (폴더 없으면 생성)
- 작업을 마치면 `docs/exec-plans/progress/` 폴더에 새 파일로 진행 상황을 저장하세요
  - 파일명 형식: `YYMMDD-HHMMSS-progress-제목.txt` (예: `260424-153012-progress-스케치이미지.txt`)
  - **파일 생성 직전에 반드시 Bash 툴로 `date +%y%m%d-%H%M%S` 를 실행하고, 그 출력값을 파일명으로 사용하세요** (시간을 추정하거나 임의로 입력하지 마세요)
  ...
  - 당일 기준 24시간 이상 지난 파일은 `docs/exec-plans/progress/old/` 폴더로 이동하세요 (폴더 없으면 생성)
...
- 답변에 필요한 추론이 완료되면 즉시 행동으로 넘어가세요. 과도한 검토나 반복 사고는 금지입니다.
- 불명확한 정보가 남아 있을 때만 질문하세요. 추측으로 채울 수 없는 경우에 한해 묻고, 나머지는 판단해서 진행하세요.
